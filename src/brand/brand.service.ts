...
import BrandModel from 'src/database/models/log.model';
import { failed } from 'src/common/dto';
import CategoryModel from 'src/database/models/orderTotalAmountHistory.model';

@Injectable()
export class BrandService {
  constructor(
    @Inject(BrandModel) private readonly brand: typeof BrandModel,
    @Inject(CategoryModel) private readonly category: typeof CategoryModel,
  ) {}

  async create(createBrandDto: CreateBrandDto) {
    try {
      const newBrand = await this.brand
        .query()
      ...
      if (
        ...
        error.nativeError.code == '23503'
      )
        throw new HttpException(
          'provided owner id deos not exists',
          HttpStatus.NOT_FOUND,
        );
      // TODO - check for unique field exception
      throw new HttpException(
        failed('Error Creating brand'),
  ...
  async findAll() {
    try {
      ...
      throw new HttpException(
        failed('Error fetching all brands'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async findOne(id: number) {
    try {
      ...
      throw new HttpException(
        failed('Error fetching brand information'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async update(id: number, updateBrandDto: UpdateBrandDto) {
    try {
      const updatedBrand = await this.brand
        .query()
  ...
  async createCategory({ brandId, payload }): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while creating category'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async geCategory(brandId: number, name: string): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while fetching category'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async fetchCategories(brandId: number): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while fetching all categories'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
