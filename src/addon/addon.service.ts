...
import AddonModel from 'src/database/models/order.model';
...
export class AddonService {
    ...
    try {
      ...
      if (
        ...
        throw new HttpException(
          `Addon with name: "${payload?.name}" already exists`,
  ...
  async get(addonId: number): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while fetching addon'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getAll(brandId: number): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while fetching all brand addons'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async update({ addonId, payload }): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while updating addon'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async delete(addonId: number): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error while deleting addon'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
