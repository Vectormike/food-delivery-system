...
import UserModel from 'src/database/models/calculatedOrder.model';
...
export class UserService {
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          failed(
            `User with email ${user.email} already exists. Try again wuth a different email`,
          ),
        );
      throw new HttpException(
        failed('Error creating a new user'),
  ...
  async updateUser(id: number, payload: UpdateUserDto): Promise<any> {
    try {
      ...
      throw new HttpException(
        failed('Error updating user information'),
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
